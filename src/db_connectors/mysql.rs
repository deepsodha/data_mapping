use std::{error::Error, collections::{HashMap, HashSet}, str::FromStr};
use super::structs::{MysqlCon, MysqlConfigFile};
...
impl MysqlCon {
    pub fn new() -> MysqlCon {
        MysqlCon { pool: None, attribute: None , role: None}
    }
}
...
#[async_trait]
impl DataAccess for MysqlCon {

    type Client = Pool<MySql>;
    ...
    async fn open_connection(&self) -> Result<Self::Client, Box<dyn Error + Send + Sync>> {
        let config = std::fs::read_to_string("mysql.json")?;
        let config_data: MysqlConfigFile = serde_json::from_str(&config)?;
        ...
        println!("mysql string- {conn_string}");
